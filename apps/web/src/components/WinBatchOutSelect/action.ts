...
import {BatchOutSelectInstanceRefs} from './types'
import { ProductSelectInstanceRefs } from '../WinProductSelect';

export class TAction extends TGridAction<BatchOutSelectInstanceRefs>{
    public getParams: object = {};
    constructor(dataSource: DataSourceType,instanceRefs: BatchOutSelectInstanceRefs){ 
        super(dataSource,instanceRefs)
         this.TableConfig = Object.assign(this.TableConfig,{
            id:'batch-out-select',sep: true,expand: false,opt:false,parger:false,readOnly: true
        }); 
        this.Columns = [{
        ...
        this.Events = $Utils.deepObjectMerge(this.Events,{
            rowDbClick: async (datas: TableRowDataType[],rowdata: TableRowDataType,rowIndex: number,cellIndex: number,fieldName: string)=>{
               const res = await $api.comonRequest(
                    ...
                    'geAllowedBatch',
                    {
                        condition: `productcode=${rowdata.productcode} and allowednum>0`,
                    }
                );
                if (res&&res.errCode==0){
                    this.instanceRefs.detailGridAction.dataSource.dataSet = res.datas;
                }
...
export class TBatchAction extends TGridAction<BatchOutSelectInstanceRefs>{
    public getParams: object = {};
    constructor(dataSource: DataSourceType,instanceRefs: BatchOutSelectInstanceRefs){ 
        super(dataSource,instanceRefs)
         this.TableConfig = Object.assign(this.TableConfig,{
    ...
    public async batchNumberMatch(curItem: TableRowDataType,target: TableRowDataType,rowIndex: number){
        if (!curItem) return;
        const MatchNum = target.allowednum>=curItem.numex?curItem.numex:target.allowednum;
        const procParmars={
                DeliveryAt: curItem.deliveryat,
                SaleIdkey: curItem.idkey,
                ProductCode: curItem.productcode,
                MatchBatchNumber: target.batchnumber,
                MatchsBatchNumber: target.batchnumber,
                MatchNum: MatchNum,
                MatchCostPrice: target.avgprice
        }
        const BatchRes = await $api.comonRequest('/masterApi/disApi/sales/sale/BillDetail/','matchBatchNumber',procParmars);
