...
import { TSalesManSaleReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'saleReports/SalesManSaleReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/SaleReports/SalesManSaleReport/',
        KeyFieldName: 'userCode',
        NotAllowOperationList: [],
        getParmars: {
            totalFieldSql: `count(userCode) as count,
...
const gridAction = reactive(new TSalesManSaleReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/SaleReports/SalesManSaleReport/', 'buildReport', {
