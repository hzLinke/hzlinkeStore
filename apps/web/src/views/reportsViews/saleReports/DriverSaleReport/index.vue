...
import { TDriverSaleReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'saleReports/DriverSaleReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/SaleReports/DriverSaleReport/',
...
const gridAction = reactive(new TDriverSaleReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/SaleReports/DriverSaleReport/', 'buildReport', {
