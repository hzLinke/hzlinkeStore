...
import { TCateSaleReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'saleReports/CateSaleReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/SaleReports/CateSaleReport/',
        KeyFieldName: 'cateCode',
        NotAllowOperationList: [],
        getParmars: {
            totalFieldSql: `count(cateCode) as count,
...
const gridAction = reactive(new TCateSaleReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/SaleReports/CateSaleReport/', 'buildReport', {
