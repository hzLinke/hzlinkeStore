...
import { TUnSaleReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'AnalysisReports/UnSaleReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/AnalysisReports/UnSaleReport/',
        KeyFieldName: 'productcode',
        NotAllowOperationList: [],
        getParmars: {
            totalFieldSql: `count(productcode) as count,
                                sum(sumnum) as sumnum,sum(week1) as week1,
                                sum(week2) as week2,sum(week3) as week3,
                                sum(week4) as week4`
        }
    })
)

const gridAction = reactive(new TUnSaleReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/AnalysisReports/UnSaleReport/', 'buildReport', {
