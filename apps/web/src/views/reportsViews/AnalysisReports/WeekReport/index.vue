...
import { TWeekReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'AnalysisReports/WeekReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/AnalysisReports/WeekReport/',
...
const gridAction = reactive(new TWeekReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/AnalysisReports/WeekReport/', 'buildReport', {
