...
import { TAbcReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'AnalysisReports/AbcReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/AnalysisReports/AbcReport/',
...
const gridAction = reactive(new TAbcReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/AnalysisReports/AbcReport/', 'buildReport', {
