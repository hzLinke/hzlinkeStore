...
import { TWtRateReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'AnalysisReports/WtRateReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/AnalysisReports/WtRateReport/',
...
const gridAction = reactive(new TWtRateReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/AnalysisReports/WtRateReport/', 'buildReport', {
