...
import { TInOutRateReport } from "./action"
import $api from '@/request/api'
import $Util from '@v4x/utils/lkxUtil'

defineOptions({
    name: 'AnalysisReports/InOutRateReport'
})
...
const dataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/reports/AnalysisReports/InOutRateReport/',
        KeyFieldName: 'productcode',
        NotAllowOperationList: [],
        getParmars: {
            totalFieldSql: `count(productcode) as count,
                                sum(purchasenum) as purchasenum,sum(saleNum) as saleNum`
        }
    })
)

const gridAction = reactive(new TInOutRateReport(dataSource, Win))
...
const handleApplay = async () => {
    ...
    const res = await $api.comonRequest('/masterApi/disApi/reports/AnalysisReports/InOutRateReport/', 'buildReport', {
