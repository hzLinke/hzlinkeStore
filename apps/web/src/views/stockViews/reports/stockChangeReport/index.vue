...
import type { StockChangeReportInstanceRefs } from './types'

defineOptions({
    name: 'reports/stockChangeReport'
})
...
const dataSource = reactive(
    new TDataSource({
            ApiPath: '/masterApi/disApi/stocks/Reports/stockChangeReport/',
            KeyFieldName: 'deliveryAt',
            NotAllowOperationList: [],
            getParmars: { totalFieldSql: 'count(productcode) as count,sum(stockNum) as StockNum,sum(num) as num,sum(changenum) as changenum,sum(SumMoney) as SumMoney' }
        }
    )
)

const instanceRefs = markRaw<StockChangeReportInstanceRefs>({
    detailDataSource: null
})

const Win = reactive(new TWinForm<StockChangeReportInstanceRefs>(instanceRefs))
