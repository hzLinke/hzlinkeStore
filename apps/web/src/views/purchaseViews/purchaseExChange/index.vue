...
import { LkxGrid, TableRowDataType } from '@v4x/ui'
...
import type { PurchaseExChangeInstanceRefs } from './types'
...
const listDataSource = reactive(
    new TDataSource({
            ApiPath: '/masterApi/disApi/purchase/purchaseExChange/BillList/',
...
const detailDataSource = reactive(
    new TDataSource({
            ApiPath: '/masterApi/disApi/purchase/purchaseExChange/BillDetail/',
            KeyFieldName: 'IdKey',
            NotAllowOperationList: [],
...
const instanceRefs = markRaw<PurchaseExChangeInstanceRefs>({
    detailVisabled,
    headerData,
    headerDisabled,
    detailDataSource,
...
const Win = reactive(new TWinForm<PurchaseExChangeInstanceRefs>(instanceRefs))
        ...
        [
            {
                ...
                dropConfig: {
                    ...
                    onFilter: (filterValue: string,dropDataList: TableRowDataType[]) => {
                        const arr = dropDataList.filter((item: any) => {
