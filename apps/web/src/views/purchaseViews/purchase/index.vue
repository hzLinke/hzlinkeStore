...
import type { PurchaseInstanceRefs } from './types'
...
const listDataSource = reactive(
    new TDataSource({
            ApiPath: '/masterApi/disApi/purchase/purchase/BillList/',
...
const detailDataSource = reactive(
    new TDataSource({
            ApiPath: '/masterApi/disApi/purchase/purchase/BillDetail/',
            KeyFieldName: 'IdKey',
            NotAllowOperationList: [],
            getParmars: {  sortFields: 'productCode,IdKey', pageSize: 0 }
        }
    )
)

const instanceRefs = markRaw<PurchaseInstanceRefs>({
    detailVisabled,
    headerData,
    headerDisabled,
    userCode,
...
const Win = reactive(new TWinForm<PurchaseInstanceRefs>(instanceRefs))
...
Win.getPrintStyle().then((res: any) => {
...
watch(detailVisabled, (n) => {
    if (n) {
        detailDataSource.getParams.sortFields = 'ProductCode,IdKey'
        headerDisabled.value = parseInt(headerData.value.mrusercode) != 0
