<template>
    ...
    <img v-if="detailVisabled&&headerData.examineusercode!=0&&headerData.over!=0" class="payPng" src="../../../assets/sk.png" />
...
<script setup lang="ts">
    ...
    const listDataSource = reactive(
        new TDataSource({
                ApiPath:'/masterApi/disApi/sales/saleReturn/BillList/',
                KeyFieldName: 'billNumber',
                NotAllowOperationList: [],
                getParmars: {totalFieldSql:'count(billnumber) as count,sum(saleMoney) as saleMoney'}
            }
        )
    );
    const detailDataSource = reactive(
        new TDataSource({
                ApiPath:'/masterApi/disApi/sales/saleReturn/BillDetail/',
                KeyFieldName: 'IdKey',
                NotAllowOperationList: [],
    ...
    const instanceRefs = markRaw<SaleReturnInstanceRefs>({
        detailVisabled,
        headerData,
        headerDisabled,
        detailDataSource,
