...
import type { SaleReplaceInstanceRefs } from './types'
import useAppStore from '@/stores';

export class TListGridAction extends TSalesListGridAction<SaleReplaceInstanceRefs>{
    constructor(dataSource: DataSourceType, instanceRefs: Partial<SaleReplaceInstanceRefs>){ 
...
export class TDetailGridAction extends TSalesDetailGridAction<SaleReplaceInstanceRefs>{
    constructor(dataSource: DataSourceType, instanceRefs: Partial<SaleReplaceInstanceRefs>){ 
        super(dataSource, instanceRefs)
        ...
        this.Events = $Utils.deepObjectMerge(this.Events,{
            rowDbClick: async (datas: TableRowDataType[],rowdata: TableRowDataType,rowIndex: number,cellIndex: number,fieldName: string)=>{
                ...
                const CurReplacenumex = (+rowdata.replacenumex)
                InputBox.show({
                    title:'代打',
                    caption:'请输入找打数量',
                    inputType: 'number',
                    note:'',
                    defaultValue:CurReplacenumex
                    }).then((res: any)=>{
                        if (res.type == 'mrOk'){
                            this.dataSource.setFieldValue('replacenumex',res.value);
                            this.dataSource.setFieldValue('numex', +this.dataSource.getFieldValue('numex') - CurReplacenumex + (+res.value))
...
export class TSplitGridAction extends TSalesDetailGridAction<SaleReplaceInstanceRefs>{
    constructor(dataSource: DataSourceType, instanceRefs: Partial<SaleReplaceInstanceRefs>){ 
        super(dataSource, instanceRefs)
        this.Columns = [{
            dataType: 'number',//数据类型
