<template>
    ...
    <LkxGrid v-if="detailVisabled" ref='lkxDetailGrid' class="grid" 
        ...
        :dataSource="detailMonthVisabled?detailMonthDataSource:detailDataSource" 
...
const listDataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/finance/Payable/Lists/',
        KeyFieldName: 'idcode',
...
const detailDataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/finance/Payable/DetailOnList/',
        KeyFieldName: 'BillNumber',
        NotAllowOperationList: [],
        getParmars: { sortFields: 'mrAt', totalFieldSql: 'count(BillNumber) as count,sum(costMoney) as CostMoney,sum(alreadymoney) as alreadymoney,sum(oweMoney) as OweMoney' }
    })
)
const detailMonthDataSource = reactive(
    new TDataSource({
        ApiPath: '/masterApi/disApi/finance/Payable/DetailOnMonth/',
        KeyFieldName: 'monthtext',
        NotAllowOperationList: [],
        getParmars: { totalFieldSql: 'count(monthtext) as count,sum(costMoney) as CostMoney,sum(alreadymoney) as alreadymoney,sum(oweMoney) as OweMoney' }
    })
)
...
const instanceRefs = markRaw<PayableInstanceRefs>({
    headerData,
    detailVisabled,
    userCode: userCode,
    detailDataSource,
...
const Win = reactive(new TWinForm<PayableInstanceRefs>(instanceRefs))
...
const handleToolsBarClick = (data: any) => {
    detailMonthVisabled.value = data.idCode == '_Month'
