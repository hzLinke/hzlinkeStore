...
import type { PayableInstanceRefs } from './types'
import { WinMoneyPayRes } from '@/components/WinMoneyPayRes';
import useAppStore from '@/stores';

export class TListGridAction extends TFinanceListGridAction<PayableInstanceRefs>{
    constructor(dataSource: DataSourceType, instanceRefs: Partial<PayableInstanceRefs>){ 
        ...
        this.TableConfig = $Utils.deepObjectMerge(this.TableConfig,{
            id:'financeViews-PayableList',sep: true,expand: false,opt:false,parger:true,readOnly: true
        }); 
        ...
        this.Events = $Utils.deepObjectMerge(this.Events,{
            ...
            rowDbClick: async (datas: TableRowDataType[],rowdata: TableRowDataType,rowIndex: number,cellIndex: number,fieldName: string)=>{
                ...
                this.instanceRefs.gridDetailAction.dataSource.getParams.mustCondition = `shipperCode=${rowdata.idcode}`;
                this.instanceRefs.gridDetailAction.open() 
                this.instanceRefs.detailMonthDataSource.getParams.condition = '';
                this.instanceRefs.detailMonthDataSource.getParams.mustCondition = `shipperCode=${rowdata.idcode}`;
...
export class TDetailGridAction extends TFinanceDetailGridAction<PayableInstanceRefs>{
    public ListColumns = [
        {
            dataType: 'text',
    ...
    constructor(dataSource: DataSourceType, instanceRefs: Partial<PayableInstanceRefs>){ 
        ...
        this.TableConfig = $Utils.deepObjectMerge(this.TableConfig,{
            id:'financeViews-PayableDetail',sep: true,expand: true,opt:false,parger:true,readOnly: true
        }); 
        this.SerachBoxConfig = $Utils.deepObjectMerge(this.SerachBoxConfig,{
            fields:[{fieldName: "billnumber",caption: "单号",filterType: "fsStrList"},
                {fieldName: "ordeerbillnumber",caption: "订单号",filterType: "fsStrList"},
                {fieldName: "username",caption: "采购员",filterType: "fsLike"},
                {fieldName: "mrat",caption: "送货日期",filterType: "fsDate"}                  
            ],
        ...
        this.Events = $Utils.deepObjectMerge(this.Events,{
            expand:(data: any)=>{
                if (data.expanded){
                    this.thisInstance.Win.ActiveGridAction = this.thisInstance.gridDetailListAction;
                    this.thisInstance.detailListDataSource.dataSet = data.rowData.detail
                } else {
                    this.thisInstance.detailListDataSource.dataSet = []
                    this.thisInstance.Win.ActiveGridAction = this;
                }
            },
            rowDbClick: async (datas: TableRowDataType[],rowdata: TableRowDataType,rowIndex: number,cellIndex: number,fieldName: string)=>{
                const res = await WinMoneyPayRes.show({
                    ...
                    companyCode:this.instanceRefs.headerData?.value.shippercode,
...
export class TDetailListGridAction extends TFinanceDetailListGridAction<PayableInstanceRefs>{
    constructor(dataSource: DataSourceType, instanceRefs: Partial<PayableInstanceRefs>){ 
        super(dataSource, instanceRefs)
        this.Columns = [{
            dataType: 'number',
        ...
        this.TableConfig = $Utils.deepObjectMerge(this.TableConfig,{
            id:'PurchaseViews-PayableDetailEx',sep: true,expand: false,opt:false,parger:false,readOnly: true,
