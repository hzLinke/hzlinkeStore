<template>
    ...
    <div class="bg">
        ...
        <div v-if="!isInitPassWord&&isAccounts" id="login">  
            ...
            <label class="input-container">
                <span>工号:</span>   
                <input type="text" required  autocomplete="new-password" placeholder="工号/手机号"  v-model="acction" autofocus @keypress="handleAcctionKeyPress"/>
            </label>
            <label class="input-container">
                <span>密码:</span>
                <input ref="passWordEl" type="password" required autocomplete="new-password" placeholder="密码"  v-model="passWord" />
            </label>
            <div class="but-bar">
        ...
        <div v-if="isInitPassWord" id="initPasword">  
            ...
            <label class="input-container">
                <span>帐号:</span>   
                <input type="text" required  autocomplete="new-password" placeholder="手机号码"  v-model="initAcction" autofocus />
            </label>
            <label class="input-container">
                <span>密码:</span>
                <input type="password" required autocomplete="new-password" placeholder="新密码"  v-model="initPassWord" />
            </label>
            <label class="input-container">
                <span>验证码:</span>
                <input type="password" required autocomplete="new-password" placeholder="验证码"  v-model="checkCode" />
        ...
        <div v-if="!isAccounts" id="accountsWin">  
            ...
            <label class="input-container">
                <span>帐号:</span>   
                <input type="text" required  autocomplete="new-password" placeholder="帐号一般是执照号"  v-model="acctionsCode" autofocus />
            </label>
            
            <label class="input-container" style="margin-top: 30px;">
                <span>验证码:</span>
                <input  required autocomplete="new-password" placeholder="验证码"  v-model="checkCode" />
...
import { LkxInput, MessageBox, type InputOptionsType } from '@v4x/ui'
...
const acctionsList = ref([
    { idcode: 0, bookname: '', appid: '', linkname: '', ltdname: '' }
])
...
const shipperState = ref<InputOptionsType>({
    config: {
        ...
        inputType: 'drop',
        onChange: (data: any) => {
            commonParam.$bookname = data.bookname
            commonParam.$linkname = data.linkname
        }
    },
    dropConfig: {
        allowInput: false,
...
const getAccounts = async () => {
    ...
    if (_AccountsCode) {
        ...
        if (res.errCode == 0 && res.datas.length != 0) {
            const acctionsInfo = res.datas[0]
            acctionsList.value = acctionsInfo.bookList
            shipperState.value.dropConfig.dropListDatas = acctionsList.value
            ...
            commonParam.$picSrc = commonParam.$picSrc + commonParam.$linkname
...
/**获取设置帐号验证码 */
const handleGetAcctionsCheckCode = async (_event: MouseEvent) => {
