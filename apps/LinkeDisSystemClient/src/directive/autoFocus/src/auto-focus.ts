/* 从vue-auto-focus 改进*/
const getTargetIndex = function(el: any,collection: any) {
 ...
 */
 const getAllFocusEls = function (el: HTMLElement,instance: any): any {
    const els = el.querySelectorAll('[data-index]') as any
    for (let i=0;i<els.length;i++){
        els[i].querySelector('input,textarea').addEventListener('focus',(e: any)=>{
            instance.currentIndex = els[i].dataset.index
        })
    }
    return els
}
 ...
 */
 const autoFocus = function (focusEl: any) {
    let elInput = focusEl
    if (['INPUT','TEXTAREA'].indexOf(focusEl.tagName.toUpperCase()) === -1) {
        elInput = focusEl.querySelector('input,textarea') as any
    }
 
    elInput && elInput.select()
 ...
 * @param el
 */
const focusCtrl = function (el: HTMLElement,instance: any) {
    const action = el.dataset.action
    const allFocusEls = getAllFocusEls(el,instance)
 ...
 export default {
     ...
     */
     mounted: function (el: any,binding: any) {
        const allFocusEls = getAllFocusEls(el,binding.instance)  // 获取需要聚焦的input元素组

        let current = getTargetIndex(el,allFocusEls)
        if (!current || current < 0 || current >= allFocusEls.length) {  // 如果没有设置data-current,或者current的数值范围不符合要求,则默认聚焦到第一个输入框
            current = 0
        }
        const currentEl: HTMLElement = allFocusEls[current]
        autoFocus(currentEl)
    },
    /**
     ...
     */
     updated: function (el: HTMLElement,binding: any) {
        if (binding.value !== binding.oldValue) {
           // binding.instance.currentIndex ++
            focusCtrl(el,binding.instance)
        }
    },
}
